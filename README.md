...
Claude Code 中使用 `/restore-knowledge`。它只归档新增或被澄清的可复用知识，不记录用户最终决策、行动安排和聊天流水账，并在 Wiki 后给出定向深入学习资源。
...
| 知识还原 | `restore-knowledge` | 把新增认知整理为 Markdown LLM Wiki，并推荐与当前认知缺口相关的深入学习资源 |
...
技能先识别本次对话的认知增量，再剔除决策、待办、个人敏感信息、重复内容和无证据猜测。输出按主题拆分为自包含 Markdown 文档，保留适用边界、证据状态、来源和关联概念。每篇 Wiki 后还会给出值得继续深入的领域以及三到五项关键学习资源，逐项说明独特视角、与当前主题的关联和建议切入方式。没有值得归档的新增知识时会直接说明，不制造伪知识或通用书单。
...
三个技能包含相互独立的评测清单，共 32 个行为用例。安装 [skill-up](https://github.com/alibaba/skill-up) 后可执行：
