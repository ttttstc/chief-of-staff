...
| 自主思考模式 | `ask-lyl` | 用户先保留自己的问题框架，人工智能再补信息、找盲区和挑战初判，最终判断仍由用户完成 |
...
自主思考不是连续反问。用户熟悉领域时，会先用简短问题框架说明目标、问题主线、事实与假设、当前初判和校准请求；已经表达的内容无需重写。用户完全陌生时，技能先补充必要概念和信息来源，不要求凭空猜方案。用户明确要求直接答案时，技能最多进行一次简短的决策权交还，随后立即给出最佳判断。
...
在人工智能展开完整问题定义和首选结论前，用户先留下自己的问题框架：目标与成功标准、当前问题模型、事实与假设、初步倾向，以及最担心判断错误的地方。它允许简短要点和“不知道”，不是考试或固定表单。用户已经表达完整时不重复询问；完全陌生时先由人工智能提供中性信息底座，信息充分后再完成框架。初判前不提供推荐选项或代填用户倾向。
...
三个技能包含相互独立的评测清单，共 38 个行为用例。安装 [skill-up](https://github.com/alibaba/skill-up) 后可执行：
...
不会。自主模式开始时可能要求一张简短的问题框架卡，但只询问尚未表达且会改变分析的部分；完全陌生时先提供信息，不要求猜测。后续问题仍按高价值批次组织。用户明确要求直接答案时，最多只进行一次简短的决策权交还，随后放行。
